...
diagnose_trend_type <- function(data) {
  transforms <- data %>%
    ...
    group_modify(~ {
      ...
      # Decide which transformation is needed
      transformation <- case_when(
        trend_label %in% c("Random Walk", "Random Walk with drift", "Random Walk with linear trend") ~ "difference",
        trend_label == "AR(1) with linear deterministic trend" ~ "detrend_linear",
        trend_label == "AR(1) with quadratic deterministic trend" ~ "detrend_quadratic",
        TRUE ~ "none"
      )
      
      # Store parameters needed to reapply or invert transformation
      params <- list()
      if (transformation == "difference") {
        params <- df %>% dplyr::filter(counter == 1) %>% pull(value)
...
create_lagged_features <- function(df, id_col, time_col, target_col, n_lags=5, numeric_features=NULL) {
  ...
  if (is.null(numeric_features)) {
    ...
    numeric_features <- setdiff(numeric_features, c(id_col, time_col))
  }
  ...
  for (lag in seq_len(n_lags)) {
    lagged_df <- df %>%
      ...
      ungroup() %>%
      select(ends_with(paste0("_lag", lag)))
    
    df_lagged <- bind_cols(df_lagged, lagged_df)
  }
  
  # Drop rows with NA introduced by lagging
...
compute_metrics <- function(y_obs, y_pred, pred_lower = NULL, pred_upper = NULL, alpha = 0.05) {
  ...
  # Winkler-Score (see formula in Hyndman & Athanasopolous, 2021)
  score <- ifelse(
    y_obs < lt,
    (ut - lt) + (2 / alpha) * (lt - y_obs),
    ifelse(
      y_obs > ut,
      (ut - lt) + (2 / alpha) * (y_obs - ut),
      ut - lt
    )
  )
  
  winkler_score <- mean(score, na.rm = TRUE)
  
  
  PI_95 <- paste0("[", lt, ", ", ut, "]")
  
  interval_width <- (ut-lt)
