...
for (id_i in unique(df_hpo$id)) { # df_hpo, because df_eval includes ids without hpo because of missings in validation set 
              ...
              test_metrics_i <- test_metrics_i %>% mutate(id = id_i, n_lags=n_lags_i, alpha=alpha_i) # add optimized HPO
...
# Plot a grid of 9 forecasts
### TODO
# TODO dafür wieder undo scaling und diff_detrend
...
########################################################################################################################
# Aber: Wert von Prädiktor x zu Zeitpunkt t kommt zwar in mehreren Zeilen vor, ABER, nicht als
# gleiches Feature, denn: Prädiktor x zu Zeitpunkt t ist in Zeile t+1 in Spalte x_lag1 und in Zeile 
# t+2 in Spalte x_lag2 etc.  
...
for (example_id in unique(test_metrics_boot$id)[6:10]) {
...
for (example_id in unique(test_metrics_boot$id)[6:10]) {
  ...
  print(
    ggplot() +
      # Prediction interval
      geom_ribbon(data = boot_plot_df,
                  aes(x = time, ymin = lower, ymax = upper),
                  fill = "red", alpha = 0.2) +
      
      # Forecast lines
      geom_line(data = boot_plot_df,
      ...
      labs(title = paste0("Regularized linear Regression with bootstrapped PI — ID ", example_id),
           x = "Time step",
           y = "Target value",
           color = "") +
      theme_minimal() +
...
for (example_id in unique(test_metrics_RFR$id)[6:10]) {
  ...
  print(
    ggplot() +
      ...
      labs(title = paste0("Random Forest Regression — ID ", example_id),
           x = "Time step",
           y = "Target value",
           color = "") +
      theme_minimal() +
